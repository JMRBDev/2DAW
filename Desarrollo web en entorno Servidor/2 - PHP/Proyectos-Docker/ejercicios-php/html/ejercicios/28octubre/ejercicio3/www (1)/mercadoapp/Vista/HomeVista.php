...
class HomeVista extends Vista{
        ...
        $this->ret .= ".bd-placeholder-img {";
            ...
            $this->ret .= "@media (min-width: 768px) {";
                $this->ret .= ".bd-placeholder-img-lg {";
                    ...
                    $this->ret .= "<a class='nav-link' href='?action=verprod'>Productos</a>";
                    $this->ret .= " </li>";
                    $this->ret .= "   <li class='nav-item'>";
   ...
   $this->ret .= "  <div class='jumbotron'>";
    ...
    /**
    * Realizamos acción Crear Categoria, method=GET
    // action=crearcat
    */
   $this->ret .="<a href=?action=crearcat class='btn btn-success'>Crear</a>";
   ...
   $this->ret .= "  </thead>";
    ...
    $categorias = $this->getModelo();
    foreach ($categorias as &$cat) {
        $this->ret .= "   <tr>";
        $this->ret .= "    <td scope='row'>".$cat->getId()."</td>";
        $this->ret .= "   <td>".$cat->getNombre()."</td>";
        $this->ret .= "   <td>".$cat->getDescripcion()."</td>";
        /**
         * Realizamos acción Modificar Categoria, method=GET
        // action=modificarcat
         */
        $this->ret .="<td><a href=?action=modificarcat&id=".$cat->getId()." class='btn btn-warning'>Modificar</a></td>";
        /**
         * Realizamos acción Borrar Categoria, method=GET
        // action=borrarcat
         */
        $this->ret .="<td><a href=?action=borrarcat&id=".$cat->getId()." class='btn btn-danger'>Borrar</a></td>";
        $this->ret .= "  </tr>";
    }
