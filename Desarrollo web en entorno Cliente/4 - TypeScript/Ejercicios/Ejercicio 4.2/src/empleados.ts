...
const numeroEmpleados: HTMLElement = document.getElementById('numero-empleados');
...
const eliminarEmpleado: Function = () => {
    let dni: string = prompt("Introduzca el DNI del empleado");
...
const modificarEmpleado: Function = () => {
    let dni: string = prompt("Introduzca el DNI del empleado");
     ...
     */
    if (empleados.some((e) => e.dni === dni)) {
        const empleado: Empleado = empleados.filter((empleado) => {
            return empleado.dni === dni;
        })[0];
        ...
        for (let propiedad in empleado) {
            if (propiedad === "numero") {
                ...
                while (true) {
                    let nuevoValor: string = prompt(`Introduzca el ${propiedad}`);
                ...
                const nuevoValor: string = prompt(`${propiedad}`);
