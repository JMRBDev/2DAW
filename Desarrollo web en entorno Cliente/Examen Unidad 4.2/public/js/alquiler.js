...
class Coche {
    ...
    alquilar() {
        ...
        let errorTxt = document.getElementById("error-txt"); // Texto de error para hacerlo invisible por si estaba visible antes
        ...
        let precioTxt = document.getElementById("precio-txt"); // Texto de precio o "factura"
        precioTxt.classList.replace('visible', 'invisible');
    }
    devolver() {
        let precioTxt = document.getElementById("precio-txt"); // Texto de precio o "factura"
        this.mostrarOcultarInfo("mostrar"); // Mostrar la info de este coche
        const diasAlquilado = Math.ceil(Math.abs(new Date().getTime() - this.fechaAlquiler.getTime()) / (1000 * 3600 * 24)); // Restar la fecha actual menos la fecha en que se alquiló el coche para hallar los días que ha sido alquilado.
        const precio = this.precio * diasAlquilado == 0 ? this.precio : this.precio * diasAlquilado; // Precio total del alquiler. He decidido poner que si el coche ha estado alquilado 0 días (es decir, algunas horas pero menos de 24), se muestre el precio de un solo día, ya que si resulta que ha sido alquilado 0 días, el precio será 0 al multiplicar <precio * 0 días>.
    ...
    mostrarOcultarInfo(mostrarOcultar) {
        const tablaInfo = document.getElementById('tabla-info'); // Tabla de información sobre el coche
        if (mostrarOcultar === "mostrar") {
            const tablaInfoBody = document.getElementById('tabla-info-body'); // Cuerpo de la tabla de información sobre el coche
...
// Lista de coches
let coches = [new Coche("23456CMN", "Fiat 500", 25, null), new Coche("1234DPR", "Seat León", 40, null)];
const alquilarDevolverCoche = (alquilarODevolver) => {
    let matricula = document.getElementById("matricula"); // Campo matrícula del formulario
    if (matricula.value) { // Si la matrícula en el formulario tiene un valor asignado:
        let errorTxt = document.getElementById("error-txt"); // Texto de error
        let cocheEncontrado;
        coches.forEach((c) => {
        ...
        if (cocheEncontrado) { // Comprobar que existe un coche con la matricula dada:
            ...
            if (alquilarODevolver.toLowerCase() === "alquilar") { // Si lo que se quiere hacer es alquilar el coche:
                if (!cocheEncontrado.fechaAlquiler) { // Si el coche encontrado no está alquilado: 
                    ...
                    let errorTxt = document.getElementById("error-txt"); // Texto de error para avisar de que el coche ya está alquilado
                ...
                if (cocheEncontrado.fechaAlquiler) { // Si el coche encontrado está alquilado: 
                    ...
                    let errorTxt = document.getElementById("error-txt"); // Texto de error para avisar de que el coche no está alquilado
                    let precioTxt = document.getElementById("precio-txt"); // Texto de precio para ocultarlo en caso de que esté mostrándose
